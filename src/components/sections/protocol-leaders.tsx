    ...
    <div className="space-y-10">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {protocolLeaders.map((protocol) => (
          <article
            key={protocol.name}
            className="group overflow-hidden rounded-3xl border border-slate-800/65 bg-surface/80 p-6 shadow-lg shadow-black/15 transition hover:border-mint/50"
          >
            <header className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-semibold text-slate-50">{protocol.name}</h3>
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-slate-300">
                  <span className="rounded-full border border-slate-800/60 bg-background/70 px-2.5 py-1 text-slate-200">
                    {protocol.chain}
                  </span>
                  <span className="rounded-full bg-background/60 px-2.5 py-1 text-slate-200">{protocol.category}</span>
                </div>
              </div>
              <div className="flex flex-col items-end gap-2 text-right">
                {'sponsored' in protocol && protocol.sponsored ? (
                  <span className="inline-flex items-center rounded-full bg-lavender/18 px-3 py-1 text-[11px] font-semibold uppercase tracking-widest text-lavender ring-1 ring-inset ring-lavender/35">
                    Sponsored
                  </span>
                ) : null}
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={clsx(
                      ...
                      changeStyles[protocol.changeDirection]
                    )}
                  >
                    <svg
                      className="h-4 w-4"
            ...
            <dl className="mt-6 grid grid-cols-[auto_1fr] gap-x-3 gap-y-2 text-sm text-slate-300">
              ...
              <dd className="font-semibold text-slate-50">{protocol.tvl}</dd>
